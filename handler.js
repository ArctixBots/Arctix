...
function printLog(isCmd, sender, msg, body, groupName, isGc) {
	...
	if (isCmd && !isGc) {
		return console.log(color("[ ARCTIX-AI ]", "aqua"), color(sender.split("@")[0], "lime"), color(body, "aqua"));
	}
}
module.exports = handler = async (m, conn, map) => {
	try {
			...
			options.adReply
				? (content.contextInfo = {
						externalAdReply: {
							...
							sourceUrl: "https://github.com/Krakinzlab/Cortana/",
						},
				  })
				: "";
			if (
				...
				fromContent.key.id = "ARCTIX" + require("crypto").randomBytes(13).toString("hex").toUpperCase();
		...
		// anti +1
		if (!isGroup && require("awesome-phonenumber")("+" + msg.sender.split("@")[0]).getCountryCode() == "234") {
			await conn.sendMessage(msg.from, { text: "My owner Doesn't allow your country code into my inbox for some reason.. ⚠️⚠️ Owner will unblock you later maybe...See You in the next life..\n*Arctix-AI ©2022Inc*·" });
			await require("delay")(3000);
			await conn.updateBlockStatus(msg.sender, "block");
			await conn.sendMessage(config.owner[0], {
				text: "*â€¢ount Block Detected Number +234*\n\nwa.me/" + msg.sender.split("@")[0],
			});
		}
